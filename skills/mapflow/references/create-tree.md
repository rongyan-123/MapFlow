...
1. Add a node. `learningObjectives` and `keyConcepts` must be JSON arrays for the current mutation implementation; `observableEvidence` is the legacy string field, so send its JSON encoding as a string.
