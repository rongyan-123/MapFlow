...
MapFlow 前端只支持下面两种展示布局,Agent 不能通过写入坐标创建第三种布局:

1. **关系布局(默认)**:前端依据有向依赖关系和学习层级自动计算位置。
2. **按块布局**:前端先按 `blocks.sortOrder` 把节点分成语义块,再在每个块内按同一套依赖关系算法排布;没有归属块的节点会留在最后的未分组区域。

Agent 修改 `positionX`、`positionY` 或 `orderInLevel` 对前端视觉布局没有决定作用,不能把节点自由拖到第三种布局。修改依赖边或学习层级会因为改变学习图语义而自然影响算法结果;这不等于 Agent 获得了自由画布。`tree.layoutMode` 以及 `update_tree` 的 `layoutMode: auto/manual` 只表示坐标写入意图,**不是**关系布局和按块布局的切换开关。

块数据是向后兼容的可选元数据。没有有效块和节点归属的旧树仍可使用关系布局,但不会出现按块布局入口;至少存在一个块和一个指向真实节点/块的归属后,前端才允许切换。并不要求所有节点都归入块。

## 背景与两条铁律

MapFlow 的树由 AI 生成,关系布局默认按「依赖/难度/推荐顺序」排布,适合顺着学。
...
`mapflow.create_tree` 可在创建请求里同时提供 `blocks` 和节点的 `blockId`。
这样树第一次打开就具备按块布局资格。块字段省略时仍可创建旧式关系树,不会因为兼容性要求而失败；但这类树在补齐块之前只能看到关系布局。
